...
router.post('/create', async (req, res) => {
  try {
      ...
      if (password.length < 6)
          return res.status(400).json({ msg: 'Pswword must be at least 6 character long!' })
      if (password !== confirmPassword)
          return res.status(400).json({ msg: 'Your password and confirmation password are not matc!h' });
